...
for layer in sketch.layers.children
	layer.onTap ->
		...
		sketch.layers.animate
			...
			options: 
				curve: "ease"
				time: 0.15 + 0.0005 * Math.abs(sketch.layers.height - newHeight)
...
makeClickableParentLayer = (layers) ->
	for layer in layers
		layer.onTap ->
			heightBeforeAnimation = componentsList.height
			componentsListPreviousHeight = componentsList.height
